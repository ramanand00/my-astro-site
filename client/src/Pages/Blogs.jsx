...
export default function Blogs() {
  ...
  useEffect(() => {
    axios.get("https://ramanand-portfolio-backend.vercel.app/api/blogs")
      .then(res => {
        setBlogs(res.data);
        setLoading(false);
      })
  ...
  return (
    <>
      ...
      <section className="min-h-[90vh] bg-gradient-to-br from-white to-gray-100 dark:from-gray-900 dark:to-gray-800 px-6 py-12">
        <motion.h1
          className="text-4xl md:text-5xl font-extrabold text-center text-indigo-700 dark:text-indigo-400 mb-8"
        ...
        <div className="max-w-2xl mx-auto mb-10">
          <input
            ...
            className="w-full px-4 py-3 rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 text-gray-800 dark:text-gray-200 focus:outline-none focus:ring-2 focus:ring-indigo-400 transition"
          />
        </div>

        <div className="max-w-7xl mx-auto grid sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredBlogs.length === 0 ? (
            <div className="col-span-full text-center text-gray-500 dark:text-gray-400 text-lg">No blogs found.</div>
          ) : (
